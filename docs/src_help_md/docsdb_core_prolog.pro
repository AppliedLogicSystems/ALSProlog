[xpge(control,'!/0 (cut)',cut0,!,[p('!/0','(cut) removes choice points')]),
    ...
    xpge(file_system,'get_cwd/1',getcwd1,get_cwd,
        [p('get_cwd/1','returns the current working directory'),
            p('change_cwd/1','change the current working directory'),
            p('canon_path/2','canonicalizes a path name'),
            p('remove_file/1',
                'removes a file from the current working directory'),
            p('filename_equal/2',
                'OS portable check for equality of file names'),
            p('getDirEntries/3','returns the list of files in a directory which match a pattern'),
            p('must_exist_file/1',
    ...
    xpge(uias,'$uia_alloc/2',uiaalloc2,'\'$strlen\'',
        [p('\'$uia_alloc\'/2','allocates a UIA of specified length'),
            ...
            p('\'$strlen\'/2','returns the length of the specified symbol')])].
