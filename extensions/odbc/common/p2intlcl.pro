...
emit_type_check(integer,Num,OutStream)
	...
	printf(OutStream,'\t\tif (!CI_get_integer((unsigned long *)&arg%d,type%d))\n',[Num,Num]),
...
emit_type_check(string,Num,OutStream)
	...
	printf(OutStream,'\telse if (!CI_get_integer((unsigned long *)&arg%d,type%d))\n',[Num,Num]),
	printf(OutStream,'\t\tPI_FAIL;\n',[]).
...
classifyThisType([union|_],integer).
...
write_compact_interf_init(State)
	...
	accessP2I(outStream, State, OutStream),
	printf(OutStream, 'extern void %t_compact_init(void);\n', [BaseName]),
	printf(OutStream, 'extern void %t2_init(void);\n', [BaseName]),
	printf(OutStream, 'extern void %t3_init(void);\n', [BaseName]),
...
gen_gv_cfunc_header(Prefix,OutStream)
	...
	printf(OutStream, '{\n',[]),
	printf(OutStream, '\tPWord dispatchVal; int dispatchType;\n',[]),
...
endmod.
mytrans(I,O) :- name(O,[0'x ,0'_ |I]).
macos_trans(I,O) :- name(O,[0'm ,0'_ |I]).
wx_trans(I,O) :- name(O,[0'w ,0'x ,0'_ |I]).
module pro2intf.
mytrans(I,O) :- name(O,[0'x ,0'_ |I]).
macos_trans(I,O) :- name(O,[0'm ,0'_ |I]).
wx_trans(I,O) :- name(O,[0'w ,0'x ,0'_ |I]).
mswin32_trans(I,O) :- name(O,[0'w ,0'_ |I]).
odbc_trans(I,O) :- name(O,[0'o ,0'_ |I]).
endmod.
