 ...
 *	A constraint solution shell solving the full
 *	"jobs" puzzle from Wos, Overbeek, Lusk, & Boyle, p. 55
 *---------------------------------------------------------------------*/


jobs :- jobs(L), write(L), nl.
...
college_educated(X) :- not(not_college_educated(X)).
...
conjecture(has_job(X, Y), Established) 
	:- 
	person(X), job(Y),
	not(member(has_job(X, Y), Established)).
...
satisfy_local_constraints(Established)
	:-
	not(exist_failed_local_constraint_for(Established)).
...
exist_failed_local_constraint_for(Established)
	...
	not(satisfy(Constraint_Head, Established)).

satisfy_global_constraints(Established)
	:-
	not(exist_failed_global_constraint_for(Established)).
