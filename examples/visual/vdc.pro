/*============================================================*
 ...
 |
 |		A desk calculator
 |			-- Visual Tcl/Tk version
 |
 | Author:  Kevin A. Buettner
 | Creation:  12/03/86
 | Revsion for Tcl/Tk: Ken Bowen (4/28/97)
 *============================================================*/
...
use tk_alslib.
	...
	%% This goal together with vdc/2 and vdc/0
	%% are the only changes/additions to the
	%% original dc.pro required to run with the
	%% visual calculator defined in vdc.tcl:
	%%
	%% To run the vdc application, execute:
	%%
	%%	?- vdc.
...
vdc(Input, Answer)
	:-
	sprintf(string(InputCodes),'%t', [Input]),
	lexan(InputCodes,Tokens),
	parse(Tokens,Tree),
	eval(Tree,Answer),
	!.
vdc(Input, bad_input)
...
readin(0'\r,[]) :- !.
readin(~\,[C|T]) :- !, 
	get0(C), get0(NextC), readin(NextC,T).
readin(C,[C|T]) :- 
	get0(NextC), readin(NextC,T).
...
e1prime(if_then_else(C,T,F),C) --> ['?'], e1(T), [':'], e1(F).
e1prime(S,S)		--> lambda.
