...
static	int	doassert	(PE, int, PWord, pword, int );
...
{				/* asserta(Module,Clause,Ref,ReconsultFlag) */
    ...
    if (xform_uia(hpe, &v1, &t1) && t4 == WTP_INTEGER &&
	doassert(hpe, IC_ASSERTA, v1, cvt_term_to_rule(hpe, v2, t2), v4)) {
...
{				/* assertz(Module,Clause,Ref,ReconsultFlag) */
    ...
    if (xform_uia(hpe, &v1, &t1) && t4 == WTP_INTEGER &&
	doassert(hpe, IC_ASSERTZ, v1, cvt_term_to_rule(hpe, v2, t2), v4)) {
	if (aib_clause_addr != (long *) 0) {
	    make_dbref(aib_clause_addr, &vdbref, &tdbref);
	    if (w_unify(v3, t3, vdbref, tdbref))
		SUCCEED;
...
{				/* addclause(Module,Clause) */
    ...
    if (xform_uia(hpe, &v1, &t1) &&
		doassert(hpe, IC_ADDCLAUSE, v1, cvt_term_to_rule(hpe, v2, t2), 0))
			SUCCEED;
    else
			FAIL;
}
...
{				/* execcommand(Goals) */
    ...
    if (doassert(hpe, IC_EXECCOMMAND, cur_mod, cvt_term_to_rule(hpe, vc, tc), 0))
	SUCCEED;
    else
	FAIL;
}
...
)
{
    ...
    if ( (status = compile_clause(hpe, rule, 0)) )
		icode(which, 0, 0, 0, 0);
...
pbi_index_proc(PE)
{
    ...
    if (xform_uia(hpe, &v1, &t1) && xform_uia(hpe, &v2, &t2) && t3 == WTP_INTEGER) {
	dbprot_t odbrs = w_dbprotect(DBRS_WRITABLE);
...
int
pbi_massively_abolish_clausegroup(PE)
{				
    PWord v1;
    int   t1;
    int   i;
...
pbi_procinfo(PE)
{
    ...
    else if (xform_uia(hpe, &v2, &t2) && xform_uia(hpe, &v3, &t3) && t4 == WTP_INTEGER) {
...
{				/* $dynamic(Module,Pred,Arity)   */
    ...
    if (xform_uia(hpe, &m, &mt) && at == WTP_INTEGER && force_uia(hpe, &p, &pt)) {
		dbprot_t odbrs = w_dbprotect(DBRS_WRITABLE);
		w_dynamic(m,p,a);
		(void) w_dbprotect(odbrs);
		SUCCEED;
...
{				/* $next_module(N,NN,Module,UseList) */
    ...
    if (t1 == WTP_INTEGER &&
	(n = next_module(hpe, (int) v1, &m, &mt, &u, &ut)) != -1 &&
...
{				
	...
	if (createModCloseProc(hpe, v1, v2, v3, v4))
		SUCCEED;
	else
		FAIL;
}
...
{				/* $listasm_ntblentry(Module,Pred,Arity)   */
	...
	mt == WTP_SYMBOL) {
	if ( (ent = w_nameprobe(m, p, (int) a)) ) {
	    list_asm(hpe, ent->overflow, (int) (NTBL_OVERFLOWSIZE 
	                               + NTBL_CALLENTRYSIZE
				       + NTBL_EXECENTRYSIZE
				       + NTBL_CODESIZE));
	    SUCCEED;
...
{				/* $firstargkey(DBRef,FirstArgKey)  */
    ...
    if ((curClause = validate_dbref(v1, t1, &procindex)) != (long *) 0) {
	lfirstargkey = *(curClause + WCI_FIRSTARGKEY);
	if (MTP_TAG(lfirstargkey) == MTP_UNBOUND)
	    w_mk_unbound(&a, &at);
	else if (MTP_TAG(lfirstargkey) == MTP_LIST) {
	    w_mk_list(&a, &at);
	    w_install_unbound_car(a);
	    w_install_unbound_cdr(a);
	}
#ifdef MTP_CONSTTAG
	else if (MTP_CONSTTAG(lfirstargkey) == MTP_INT) {
	    a = MINTEGER(lfirstargkey);
	    at = WTP_INTEGER;
	}
#else  /* MTP_CONSTTAG */
	else if (MTP_TAG(lfirstargkey) == MTP_INT) {
	    a = MINTEGER(lfirstargkey);
	    at = WTP_INTEGER;
	}
#endif /* MTP_CONSTTAG */
	else {			/* A functor or a symbol */
	    tokid = MFUNCTOR_TOKID(lfirstargkey);
	    arity = MFUNCTOR_ARITY(lfirstargkey);
	    if (arity == 0) {	/* symbol */
		a = tokid;
		at = WTP_SYMBOL;
	    }
...
pbi_resolve_module(PE)
{
    ...
    if (xform_uia(hpe, &v1, &t1) && modprobe_id(hpe, v1) != -1 &&
	xform_uia(hpe, &v2, &t2) && t3 == WTP_INTEGER) {
	...
	if ((ent = w_nameprobe(v1, v2, (int) v3)) &&
	    ...
	    (ent->flags & NMSK_USAGE) != NFLG_IMPORTED) ;	/* ok, we got
								 * it locally
								 */
	else if ((ent = resolve_ref(hpe, v1, v2, (int) v3)))	/* try looking for it
							 */
		...
		(ent->flags & NMSK_USAGE) != NFLG_IMPORTED) ;	/* found
								 ...
								 * locally
								 */
	    else if (!(ent = resolve_ref(hpe, v1, v2, (int) v3 - 1)))
		FAIL;
	}
	else
	    FAIL;
...
{				/* push_clausegroup(CG) */
    ...
    push_clausegroup(hpe, v1);
    SUCCEED;
}

int
pbi_pop_clausegroup(PE)
{				/* pop_clausegroup(CG) */
    PWord v1;
    int   t1;
    ...
    if (w_unify(v1, t1, (PWord) pop_clausegroup(hpe), WTP_INTEGER))
	SUCCEED;
    else
	FAIL;
}
