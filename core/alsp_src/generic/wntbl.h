/*
 * wntbl.h	-- machine specific name table entry sizes for wintcode.h
 ...
 *
 *
 * NTBL_HEADERSIZE gives the number of words occupied by the structure prior
 * to the overflow entry code.
 *
 * NTBL_OVERFLOWSIZE gives the number of words needed for a jsr to the overflow
 * code.
 *
 * NTBL_CALLENTRYSIZE gives the number of words needed for the link instruction
 * at the call entry point.
 *
 * NTBL_EXECENTRYSIZE gives the number of words needed for the overflow checking 
 * code.  This, coincidentally, is also the number of words needed for a jsr
 * to the spy handler.
 *
 * NTBL_CODESIZE is the total number of words for the rest of the entry point
 * which will include the conditional branch to the overflow code and any
 * other stuff which needs to happen afterwards.
 *
