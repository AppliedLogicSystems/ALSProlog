/*
 * fatal.c      -- fatal error messages and function
...
static char *errors[] =
{
    ...
    "error opening /dev/zero while allocating heap and stack",	/* FE_DEVZERO */
    ...
    "Memory exhausted while allocating clause space.",
							/* FE_XMEM_CLAUSE */
    "Internal GCMAGIC error (code space gc), addr=%x.",
							/* FE_GCMAGIC_CGC */
    "Internal GCMAGIC error (regular gc), addr=%x.",
							/* FE_GCMAGIC */
    "Module table is full.",	/* FE_FULL_MODTBL */
    "Too many nested modules (Module stack overflow).",
							/* FE_OVER_MODSTK */
    ...
    "Internal error while initializing builtins table.",
							/* FE_IN_BLTINIT */
 ...
 "Internal error #8 in packaging code (Illegal package symbol entry type).",
    ...
    "Internal error #10 in packaging code (Illegal relocation information type).",
							/* FE_IN_PCKG10 */
    "Internal error #1 in w_get (Unknown constant %x).",
							/* FE_IN_WGET1 */
    "Internal error #2 in w_get (Unrecognized type %x).",
							/* FE_IN_WGET2 */
