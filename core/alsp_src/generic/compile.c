				 ...
				 */
{
    ...
    /* Figure out how big of an environment we need after each call...results
     * are left in the call_env_sizes array
     */
    ...
    pword rule;
{
	...
	/*
	 * Get the functor id of the goal.  For example, the functor id in
	 * p(_,a,b) would be the symbol table index representing p.
	 *
	 * If a valid functor id can not be obtained, then the goal is something
	 * like a number which is invalid, so we return with an unsuccessful
	 * completion code from this function.
	 */
	...
	if (isarithmetic(goal)) {
	    arith_temp_homes();
	    comp_math(goal, firstgoalnumber == lastgoalarithmetic, regusemap(),
		      model_E - model_SP);
	...
	/*
	 * Put out the macro code if inline expansion code was generated for
	 * the goal.
	 */
	...
	/*
	 * Scan over all of the upcoming cuts, emit the cut code and set
	 * the isdeterminateforsure flag if a cut was seen.
	 */
	...
	if (goalnumber < ngoals) {
	    ...
	    /* When compiling a math expression, we want to use the size of
	     * the environment in the previous goal in order to avoid
