/*
 * lexan.h              -- lexical analyzer (include file) for prolog system
 ...
 *                                         management
 *      Revision: 8/8/85,   K. Buettner -- split lexical analyzer and parser
 *                                         into two files; rewrote parts of
 *                                         it for both versatility and
 *                                         efficiency
...
typedef struct _lex_buf {
            ...
            void (*nextbuf) PARAMS(( struct _lex_buf * ));   
	    			/* function responsible for getting next
                                   buffer                       */
            int (*err_rec) PARAMS(( struct _lex_buf *, char * ));
	    			/* function responsible for error message/
                                   recovery                     */
