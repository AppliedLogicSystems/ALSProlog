...
/*
 * bench.pro		-- equation solving benchmark
 ...
 *	equation and report on the time taken by the solver.
 *
 * Program Author: Keith Hughes
 * Creation:	Sometime in the 80's.
 *
 * Usage:
 *	?- reconsult(bench).
 *	?- main.
...
lc_semi(X,Term,STerm,Pos,End,Term) :-
	leastCover0(X,Term,Pos,End).
lc_semi(X,Term,STerm,Pos,End,Least) :-
	leastCover(X,STerm,Least).
...
% this may look ugly, but it keeps unsightly axioms out of the axiom set
	% (Old=Inter -> Old=New ; canonical(Inter,New)).
...
isolax(1,-U=V,U= (-V)).
isolax(1,V + U = W, V = W + (-U)).
isolax(2,V + U = W, U = W + (-V)).
isolax(1,V * U = W, V = W / U).
isolax(2,V * U = W, U = W / V).
isolax(1,V / U = W, V = W * U).
isolax(2,V / U = W, U = V / W).
isolax(1,X ^ Y = Z, X = Z ^ (1 / Y)).
isolax(2,Y ^ X = Z, X = log(Y,Z)).
isolax(1,log(Base,Y) = Exp, Base = Y ^ (1 / Exp)).
isolax(2,log(Base,Y) = Exp, Y = Base^Exp).
isolax(1,ln(Y) = Exp, Y = e^Exp).
isolax(1,sin(U)=V,U=n0*180+ (-1)^N*arcsin(V)).
isolax(1,cos(U)=V,U=n0*180+arccos((-1)^N*V)).
isolax(1,tan(U)=V,U=arctan(V)).
isolax(1,arcsin(U)=V,U=sin(V)).
isolax(1,arccos(U)=V,U=cos(V)).
isolax(1,arctan(U)=V,U=tan(V)).
isolax(1,sinh(U)=V,U=arcsinh(V)).
isolax(1,cosh(U)=V,U=arccosh(V)).
isolax(1,tanh(U)=V,U=arctanh(V)).
isolax(1,arcsinh(U)=V,U=sinh(V)).
isolax(1,arccosh(U)=V,U=cosh(V)).
isolax(1,arctanh(U)=V,U=tanh(V)).
