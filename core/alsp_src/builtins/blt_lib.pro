:-builtins:libhide(builtins,['library',misc_io],
    [copyFiles/2,copy_file/2,readline/1,readline/2,read_terms/1,read_terms/2,
        writeListNl/1, colwrite/4, putc_n_of/3,
	gen_file_header/3,gen_file_header/4]).

:-builtins:libhide(builtins,['library',listutl1],
...
:-builtins:libhide(avl,['library',avl],
    [avl_create/1,avl_inorder/2,avl_inorder_wdata/2,avl_insert/4,avl_search/3]).

:-builtins:libhide(windows,['library',simplio],
    [simple_menu/3, fin_simple_menu_code0/5 ]).

:-builtins:libhide(windows,['library',iolayer],[chooseFile/3,menu/3,menu/4]).

:-builtins:libhide(builtins,['library',commal],[flatten_comma_list/2]).

:-builtins:libhide(builtins,['library',misc_db],
	[assert_all/2,assert_all_refs/3,erase_all/1,abolish_list/1,
	 abolish_list/2]).
:-module_closure(assert_all,1), module_closure(assert_all_refs,2).

:-op(600,xfy,':=').
:-builtins:libhide(objects,['library',objs_run],
...
:-builtins:libhide(builtins,['library',typecomp],
			[comptype_cl/0, comptype/0, comptype/1]).

:-builtins:libhide(objects,['library',objects],
