From vellino@bnr.caFri Oct 20 19:49:28 1995
Date: Fri, 20 Oct 1995 17:02:00 -0400 
...
next (and prev) return the fp after (and before) the argument
If there is no OS support, we use the fact the IEEE doubles have
the mantissa as the last bits in the 64 bits, as in
...
To get the next number, we simply increment the mantissa. Since 
the mantissa is larger than 32 bits, it may carry, so we check and 
increment the top 32 bits if it does. (prev simply decrements)

****************************************************************************/

#define __ieeenext(x)	if (x GE 0.0) { \
...
#define __ieeeprev(x)	if (x GT 0.0) { \
							if (--(((fpoverlay *)&x)->l[SECOND]) EQ -1) \
								--(((fpoverlay *)&x)->l[FIRST]); \
							} \
						else if (x LT 0.0) { \
...
#ifdef __mpw
#ifndef __powerc										/* MPW C 68K */
#include <SANE.h>
#include <float.h>			/* for DBL_MAX */
					/* MPW C on the Macintosh */
...
#define roundnearest(x) setenvironment(BNRP_SANE_default_environment); x = rint(x)
...
#else													/* MPW C PowerPC */
#include <float.h>			/* for DBL_MAX */
#include <fp.h>				/* most math stuff + nextafter */
#include <fenv.h>			/* for rounding functions */
#define int_fp			fp
#define iszero(x)		(x EQ -0.0)
#define next(x)			x = nextafterd(x, maxfp)
#define prev(x)			x = nextafterd(x, -maxfp)
#define roundnearest(x) fesetenv(FE_DFL_ENV); x = rint(x)
#define upperbd(y, x)	fesetround(FE_UPWARD); x = y
#define lowerbd(y, x)	fesetround(FE_DOWNWARD); x = y
#define upperbd2(y, x)	fesetenv(FE_DFL_ENV); x = y; next(x)
#define lowerbd2(y, x)	fesetenv(FE_DFL_ENV); x = y; prev(x)
#define initfpu()		maxfp = DBL_MAX
#define resetfpu()		fesetenv(FE_DFL_ENV)
#endif	/* __powerc */
#endif	/* __mpw */
...
#ifdef __MWERKS__									/* CodeWarrior on Macintosh */
#include <float.h>			/* for DBL_MAX */
#define int_fp			fp
#define iszero(x)		(x EQ -0.0)
#ifdef powerc
...
#else
#include <SANE.h>
#define next(x)         NextDouble(&x, &maxfp)
#define prev(x)         { fp __maxfp = -maxfp; NextDouble(&x, &__maxfp); }
#define roundnearest(x) SetEnvironment(&BNRP_SANE_default_environment); x = rint(x)
#define upperbd(y, x)   SetRound(Upward); x = y
#define lowerbd(y, x)   SetRound(Downward); x = y
#define upperbd2(y, x)  SetEnvironment(&BNRP_SANE_default_environment); \
                        SetRound(Upward); SetPrecision(DblPrecision); { double _t = y; x = _t; }
#define lowerbd2(y, x)  SetEnvironment(&BNRP_SANE_default_environment); \
                        SetRound(Downward); SetPrecision(DblPrecision); { double _t = y; x = _t; }
#define initfpu()       ProcEntry(&BNRP_SANE_default_environment); SetPrecision(DblPrecision); maxfp = DBL_MAX
#define resetfpu()      ProcExit(&BNRP_SANE_default_environment)
Environment BNRP_SANE_default_environment;
#endif
...
#define roundnearest(x) fpu("set", "direction", "nearest"); x = rint(x)
...
#ifdef solaris										/* SPARCSTATION under SOLARIS */
#include <values.h>				/* for DBL_MAX */
#include <ieeefp.h>
#define int_fp			fp
#define next(x)			x = nextafter(x, maxfp)
...
#ifdef ultrix										/* DECstation 3100 */
#include <values.h>				/* for DBL_MAX */
#include <mips/fpu.h>
#define int_fp			fp
#define iszero(x)		(x EQ -0.0)
#define next(x)			__ieeenext(x)
#define prev(x)			__ieeeprev(x)
...
#ifdef nav                                                                                      /* Motorola 88000 SysV4 - NAV */
#include <values.h>                                     /* for DBL_MAX */
#include <ieeefp.h>                                     /* for fpsetround() */
#define int_fp                          fp
#define iszero(x)                       ((x EQ -0.0) || (x EQ 0.0))
#define roundnearest(x)         resetfpu(); x = rint(x)
#define next(x)                         __ieeenext(x)
#define prev(x)                         __ieeeprev(x)
#define upperbd(y, x)           fpsetround(FP_RP); x = y
#define lowerbd(y, x)           fpsetround(FP_RM); x = y
...
#define initfpu()               resetfpu(); maxfp = DBL_MAX
#define resetfpu()              fpsetround(FP_RN);
#endif  /* nav */


#ifdef mdelta                                                                           /* Motorola 68000 SysV3 */
#include <values.h>                                     /* for DBL_MAX */
#define int_fp                          fp
#define iszero(x)                       ((x EQ -0.0) || (x EQ 0.0))
#define next(x)                         __ieeenext(x)
...
#define roundnearest(x)         asm("fmov.l &0x80, %fpcr"); x = rint(x)
...
#define roundnearest(x)         { double val; \
                                if (x GE -0.0) { \
                                        val = ceil(x); \
                                        if(0.5 EQ (val - x)) \
                                                x = (iszero(fmod(val,2.0))) ? val : floor(x); \
                                        else \
                                                x = (long)(x + 0.5); \
                                } \
                                else { \
                                        val = floor(x); \
                                        if(-0.5 EQ (val - x)) \
                                                x = (iszero(fmod(val,2.0))) ? val : ceil(x); \
                                        else \
                                                x = (long)(x - 0.5); \
                                }}
...
#ifndef int_fp										/* unknown machine, try some defaults */
#endif
...
/*Boolean operation tables to convert booleanInput --> booleanOutput*/
	...
	static int op_negation[64]={0x00,0x80,0x0c,0x08,0x80,0x80,0x80,0x80,
       ...
       static int op_conditional[64]={0x00,0x80,0x2C,0x00,0x80,0x80,0x80,0x80,
                               0x43,0x80,0xff,0x40,0x00,0x80,0x20,0x30,
                               0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,
                               0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,
                               0x00,0x80,0x0C,0x08,0x80,0x80,0x80,0x80,
                               0x03,0x80,0xff,0x00,0x02,0x80,0x00,0xff,
	...
	{       long *tempfunctor;
		...
		{/* If tcb->args[1] is a list, place all items in the list on the queue*/
			...
			case LISTT:
				qhead = NULL;
				qtail = (long *)&qhead;
			/*	nextArg((long *)functor,t); */ /* mod made by Jason Harris Feb. 95 for 'cc' compatibility*/

				tempfunctor = (long *)functor; /* use tempfunctor to avoid cc problem with (long *) functor */
				nextArg(tempfunctor, t);
				functor = (long)tempfunctor;
				while((*(long *)(t = addrof(t))) EQ (STRUCTHEADER | 4)) /*While object at the front of the list is a structure, queue it*/
				{
					...
					/* nextArg((long *)functor,t); */ /* mod made by Jason Harris Feb. '95 for 'cc' compat.*/
					tempfunctor = (long *)functor; /* use tempfuctor to avoid cc problem with (long *) functor */
					nextArg(tempfunctor, t);
					functor = (long)tempfunctor;
	...
	dump("bz", z);
			...
			switch (nameof(functor)[2]) {
				...
				case do_not:
					...
					break;
                                case do_cond:
                                        booleanOutput=op_conditional[booleanInput];
					break;
				default:
					goto error;
			}
