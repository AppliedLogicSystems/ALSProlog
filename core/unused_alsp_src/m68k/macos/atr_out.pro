...
module output.
    ...
    output_begin(File) :-		% For MPW Asm, output necessary include files
		printf("     case on\n"),
    ...
    output_end(File) :-			% For MPW Asm, output necessary terminator for
...
%
    ...
    operand(indir(An),Tab) :- !, write('('), write(An), write(')').
    operand(postincr(An),Tab) :- !, write('('), write(An), write(')+').
    operand(predecr(An),Tab) :- !, write('-('), write(An), write(')').
...
stripLeading0([0'0 | Rest], Stripped) :- !, stripLeading0(Rest,Stripped).
stripLeading0([],"0").
