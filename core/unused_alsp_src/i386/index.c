...
#define RETRYSIZE 3	/* size of a retry or trust instruction in long words */
#define TBOVERHEAD 3	/* overhead required for table entry in	long words */
...
typedef struct index_str {
	...
	/* Either machine type this node represents or number of entries if
	   tag is TOP */
	int key;
...
#define TRY	((CodePtr) wm_try)
#define RETRY	((CodePtr) wm_retry)
#define TRUST	((CodePtr) wm_trust)
...

{
	...
	/* No sense indexing if only not multiple clauses or already indexed. */
	...
	/* Only output switch_on_term if at least one type needs an index
	   block. */
...
*/
	} else {	/* Must not need the indexing block. Let it go. */
		if (totalsize != 0) {
...

{
	...
	if (s == NULLNODE) {
		...
		/* set start addr to 0 when there are no clauses of the given
		   type.  The switch on term installer will convert this into
		   a jump to fail */
	...
	if (toplevelcount == 1) {
		...
		/* no need to do indexing, current chain naive is already
		   sufficient
		*/
...

{
	CodePtr p;
	CodePtr **instr;
	extern CodePtr *swConstPtr, *swStructPtr;
	...
	*p++ = 0xff;			/* CALL [instr]		*/
	*p++ = 0x15;
	*(LongPtr(p))++ = (long)instr;	/* wm_sw_term or wm_sw_const */
...
int *key;		/* Actual Prolog type of 1st argument */
...

{
	...
	*key = k = *(addr + WCI_FIRSTARGKEY);
	...
	switch (k & MTP_TAGMASK) {
		case MTP_UNBOUND : *tp = ITP_VAR; break;
		case MTP_LIST    : *tp = ITP_LIST; break;
		case MTP_CONST   : 
