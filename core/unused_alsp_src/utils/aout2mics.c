/*==================================================================*
 ...
 |
 |  	Merge Image and saved Code State (a.out version) #2
 |		-- For HP 9000/700/800
 |
 | Author: Kevin A. Buettner
 | Created: 10-14-93
 | Revised from aoutmics.c by Ken Bowen, June 1995
 *==================================================================*/

/* #include <a.out.h>  -- used by aoutmics.c */
#include <filehdr.h> 
#include <aouthdr.h> 
...
#   define oiname argv[3]
    ...
    if ( lseek(oifd, 0, 0) < 0
	|| read(oifd, &hhdr, sizeof hhdr) < 0)
	fe("Cannot read main header information", 0);

printf("hhdr.vid=%d  auxh=%x \n",hhdr.version_id, hhdr.aux_header_location);
exit(1);
