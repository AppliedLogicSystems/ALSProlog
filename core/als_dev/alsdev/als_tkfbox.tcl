# tkfbox.tcl --
#
#	ALS Modifications: Use diff with the original to find the
#	changes; they support the directory "browse" mode of tkFDialog
#	which is used to change current directory without trying to
#	deal with any files...
#
...
proc tkFDialog {args} {
	...
	set type open
	} elseif ![string compare [lindex [info level 0] 0] tk_getSaveFile] {
	set type save
    } else {
	set type browse
    }
...
proc tkFDialog_EntFocusIn {w} {
	...
	} elseif ![string compare $data(type) save] {
	$data(okBtn) config -text "Save"
	} else {
	$data(okBtn) config -text "Here"
    }
}
