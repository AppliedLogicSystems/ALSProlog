...
defStruct(server_info, [
	propertiesList = [
		...
		host_ip/'0.0.0.0',		%% server's host ip address
		host_name/'noname',		%% server's host name
		self_start/'',			%% command to start self
		min_workers/1,
...
%		idle_machines/[],       %% Idle processes already running
		master_host/nil,		%% IP of (controlling) master host, if any
		master_port/nil,		%% Port num of (controlling) master host, if any
		worker_id/nil,			%% ID# of local server process 
								%% 		as worker for master
%		master_RS/nil,			%% Read socket to master host
%		master_WS/nil,			%% Write socket to master host
