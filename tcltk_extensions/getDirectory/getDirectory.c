 ...
 *  getDirectory 
 *
 *	This file is the platform independed portion of the getDirectory
 *  command for Tcl/Tk.
 *
...
#pragma unused(clientData)
	...
	result = GetDirectory(interp, &initdir, parent, prompt);
...
__declspec(dllexport) int Getdirectory_Init(Tcl_Interp *interp)
{
    ...
    if (!Tcl_CreateObjCommand(interp, "getDirectory", getDirectory_ObjCmd, NULL, NULL))
    	return TCL_ERROR;
    
    return Tcl_PkgProvide(interp, "getDirectory", VERSION_STRING);
}

