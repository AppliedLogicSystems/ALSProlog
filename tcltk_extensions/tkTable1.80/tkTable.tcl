# table.tcl --
#
# version 1.3, jhobbs@cs.uoregon.edu
...
# FIX: It ignores a user-specified cursor setting
#
proc tkTableCheckBorder {w x y} {
    set cursor [$w cget -cursor]
    if {[string match {*[0-9]*} [$w border mark $x $y]]} {
	if {[string comp crosshair $cursor]} { $w config -cursor crosshair }
    } else {
...
bind Table <BackSpace> {
    set temp [%W icursor]
    if {[string comp {} $temp] && $temp} {%W delete active [expr $temp-1]}
    catch {unset temp}
}
